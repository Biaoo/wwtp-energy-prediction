...
class WWTPPredictor:
    ...
    def predict(self, input_data):
        ...
        try:
            ...
            for col in ['treatment_process', 'advanced_processs', 'disinfection_processs']:
            ...
            categorical_features = ['treatment_process', 'advanced_processs', 'disinfection_processs']
            for col in categorical_features:
                if col in data_with_features.columns:
                    # Use pd.get_dummies to encode (same as training)
                    dummies = pd.get_dummies(data_with_features[col], prefix=col, dummy_na=True)
...
- **测试集R²**: {predictor.model_config.get('test_metrics', {}).get('r2', 'N/A') if predictor.model_config else 'N/A'}
    ...
    with gr.Blocks(title="污水处理厂能耗预测系统") as interface:
        ...
        with gr.Row():
            with gr.Column():
                ...
                treatment_process = gr.Dropdown(
                    label="主体处理工艺",
                    choices=['None', 'A2O_Family', 'AO_Family', 'Oxidation_Ditch', 
                            'SBR_Family', 'MBR', 'Biofilm', 'Other'],
                    value='A2O_Family'
                )
                advanced_process = gr.Dropdown(
                    label="深度处理工艺",
                    choices=['None', 'Membrane', 'Filtration', 'Sedimentation', 
                            'Combined', 'Other'],
                    value='None'
                )
        ...
        predict_btn.click(
            fn=predict_energy,
            inputs=[
                treatment_capacity, annual_treatment_volume,
                treatment_process, advanced_process, disinfection_process,
                cod_influent, bod5_influent, ss_influent,
                nh3n_influent, tn_influent, tp_influent,
                cod_effluent, bod5_effluent, ss_effluent,
                nh3n_effluent, tn_effluent, tp_effluent
            ],
            outputs=output
        )
